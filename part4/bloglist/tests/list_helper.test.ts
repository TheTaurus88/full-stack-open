...
describe('most blogs', () => {
  ...
  test('when list has only one blog, equals there is only one author', () => {
    const result = listHelper.mostBlogs(listWithOneBlog)
    assert.deepStrictEqual(result, { 'author': listWithOneBlog[0].author, 'num': 1})
  })

  test('of a bigger list is calculated right', () => {
    const result = listHelper.mostBlogs(blogs)
    assert.deepStrictEqual(result, { author: 'Robert C. Martin', num: 3 })
  })
})
